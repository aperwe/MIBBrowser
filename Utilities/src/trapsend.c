...
static const char rcsid[] = "$Id: trapsend.c,v 1.38.4.1 1998/03/19 14:22:27 partain Exp $";
...
trapsend.c
     ...
     and the integer number that corresponds to the trap to be sent.
     For example:

          trapsend -v[1|2c] suzzy public 0   (SNMPv1 community string)

                    or

          trapsend -v2 suzzy Guest coldStart   (SNMPv2 clustername)
...
extern int EntryPoint_TrapSend (int argc, char *argv[]);
...
extern int EntryPoint_TrapSend (int argc, char *argv[])
{
     ...
     * snmpTrapOID.0.  If a LINK_UP, LINK_DOWN, or EGP_NEIGHBOR_LOSS trap
     * is being sent, extra varbinds would be sent (see RFC 1450) in the
     * /doc/snmpv2 directory of your distribution).
     */
