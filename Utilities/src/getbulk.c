...
static const char rcsid[] = "$Id: getbulk.c,v 1.39.4.1 1998/03/19 14:22:17 partain Exp $";
 ...
 *       getbulk -v2 suzzy Guest 1 4 iso iso
 *
 ...
 *
 *   Getbulk only works with SNMPv2.
 */
...
//                  char *argv[]));
extern int EntryPoint_GetBulk (int argc, char *argv[]);
...
extern int EntryPoint_GetBulk (int argc, char *argv[])
{
  ...
  retry_snmp_request:
    ...
    if (resp_pdu_ptr->u.normpdu.error_status != NO_ERROR) {
        ...
        printf("number of received varbinds is "INT32_FORMAT"\n",
               (resp_pdu_ptr->num_varbinds));
