...
static const char rcsid[] = "$Id: inform.c,v 1.37.4.1 1998/03/19 14:22:23 partain Exp $";
...
inform.c
        ...
        An InformRequest PDU is a new type of PDU introduced with SNMPv2. It
is used by a SNMPv2 entity that wishes to notify another application (a
SNMPv2 entity also acting in a manager role) of information in the MIB view
of a party local to the sending application.  This utility shows the user
how to send InformRequest PDUs.  The arguments are the entity's address,
the clustername for a group of SNMP parties and contexts (cluster)
...
inform -v2 suzzy Guest snmpEventID.2 snmpRisingAlarm 4

would send a message containing the following varbinds(name and value):
      sysUpTime.0, snmpEventID.2, and the matching entries in the snmpAlarmEntry
      table for - snmpAlarmVariable, snmpAlarmSampleType, snmpAlarmValue, and
...
extern int EntryPoint_Inform (int argc, char *argv[]);
...
extern int EntryPoint_Inform (int argc, char *argv[])
{
    ...
    /*
     * If a SNMPv2 entity was really sending an InformRequest PDU, access
     * checks would be made on all varbinds (see trapsend.c for an example)
     * and an extra set of varbinds would be sent if the value for
     * snmpEventID.i was: snmpRisingAlarm, snmpFallingAlarm, or
