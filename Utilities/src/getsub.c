...
static const char rcsid[] = "$Id: getsub.c,v 1.31.4.1 1998/03/19 14:22:22 partain Exp $";
...
getsub.c
     ...
     munity string to provide access to that entity or a clustername for
     a group of SNMP parties and contexts (cluster), and, optionally,
     a net or subnet address qualifier. Only those portions of the
     ...
     getsub -v[1|2c] myagent topSecret 192.244        (SNMPv1 community string)
                   or
     getsub -v2 myagent Guest 192.244         (SNMPv2 clustername)
...
extern int EntryPoint_GetSub (int argc, char *argv[]);
