...
/* $Id: v2table.h,v 1.17 1998/01/08 02:37:18 mark Exp $ */
...
/* prototypes */
int CopyEntries
    SR_PROTOTYPE((const int maxNominator,
		  const SnmpType *typep,
		  void *newvalue,
		  const void *oldvalue));

void TransferEntries
    SR_PROTOTYPE((const int nominator,
                  const SnmpType *typep,
                  void *newp,
                  void *oldp));
...
int SearchTable
    SR_PROTOTYPE ((const SnmpV2Table *rtp,
                  int st));

int NewTableEntry
    SR_PROTOTYPE((SnmpV2Table *rtp));
